...
def evaluate_pragmatics(model, dataset, batch_size, vae, glove_data, fieldname, num_dist=None):
    ...
    for _ in range(num_test_batches):
        with torch.no_grad():
            speaker_obs, listener_obs, labels, _ = gen_batch(dataset, batch_size, fieldname, p_dropout=0, vae=vae, glove_data=glove_data, see_distractors=settings.see_distractor, num_dist=num_dist)
...
def evaluate_lexsem(model, dataset, batch_size, vae, glove_data, fieldname, num_dist=None):
    ...
    for _ in range(num_test_batches):
        with torch.no_grad():
            speaker_obs, listener_obs, labels, _ = gen_batch(dataset, batch_size, fieldname, p_dropout=1, vae=vae, glove_data=glove_data, see_distractors=settings.see_distractor, num_dist=num_dist)
            outputs, _, _, recons = model(speaker_obs, listener_obs)
            recons = torch.squeeze(recons, dim=1)
        pred_labels = np.argmax(outputs.detach().cpu().numpy(), axis=1)
        num_correct += np.sum(pred_labels == labels.cpu().numpy())
...
def eval_model_pragmatics(model, vae, comm_dim, data, viz_data, glove_data, num_cand_to_metrics, save_eval_path,
    ...
    if calculate_complexity:
        test_complexity = get_info(model, data, targ_dim=comm_dim, p_dropout=0, glove_data=glove_data, num_epochs=200)
...
def eval_model_lexsem(model, vae, comm_dim, data, viz_data, glove_data, num_cand_to_metrics, save_eval_path,
    ...
    if calculate_complexity:
        test_complexity = get_info(model, data, targ_dim=comm_dim, p_dropout=1, glove_data=glove_data, num_epochs=200)
        print("Test complexity", test_complexity)
    else:
        test_complexity = None
        val_complexity = None
...
#    print(alignment_datasets[0])
    ...
    for p_train in p_dropouts:
        print(p_train)
        model_to_eval_path = 'src/saved_models/with_ctx/kl_weight0.01/p_dropout' + str(p_train) + '/seed0/2999' if settings.with_ctx_representation else 'src/saved_models/without_ctx/kl_weight0.01/p_dropout' + str(p_train) + '/seed0/2999'
