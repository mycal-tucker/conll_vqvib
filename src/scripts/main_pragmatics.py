...
def run():
    ...
    for p in settings.prob_notseedist:
        ...
        folder_training_type = "train_dropout/p_dropout"+str(p)+"/" if settings.dropout else "train_probab/notsee_prob"+str(p)+"/"
        save_loc = 'src/saved_models/' + folder_training_type + folder_ctx + 'kl_weight' + str(settings.kl_weight) + '/seed' + str(seed) + '/' 
        train(model, train_data, val_data, viz_data, glove_data=glove_data, p_notseedist=p, vae=vae_model, savepath=save_loc, comm_dim=c_dim, fieldname='topname', num_epochs=n_epochs, batch_size=b_size, burnin_epochs=num_burnin, val_period=v_period, plot_comms_flag=do_plot_comms, calculate_complexity=do_calc_complexity)
